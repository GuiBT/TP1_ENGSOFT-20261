...
def init_db():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE usuarios (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT UNIQUE NOT NULL, papel TEXT NOT NULL)')
    c.execute('CREATE TABLE recursos (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL)')
    c.execute('CREATE TABLE salas (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT UNIQUE NOT NULL, capacidade INTEGER NOT NULL)')
